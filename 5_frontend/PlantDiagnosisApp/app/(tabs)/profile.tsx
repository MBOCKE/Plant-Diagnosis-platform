...
export default function ProfileScreen() {
  ...
  const menuItems: MenuItem[] = [
    { icon: 'language', label: 'Preferred Language', value: 'Français' },
    { icon: 'location', label: 'My Location', value: 'Foumbot, West Region' },
      ...
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        ...
        <Card className="mb-8 p-0 overflow-hidden">
          {menuItems.map((item, i) => (
            <TouchableOpacity
              key={i}
              className="flex-row items-center justify-between px-5 py-4 border-b border-[#F5F5F5] last:border-0"
            >
              <View className="flex-row items-center gap-3">
                <Ionicons name={item.icon} size={22} color="#757575" />
                <Text className="text-[#212121]">{item.label}</Text>
              </View>
              {item.toggle ? (
                <View className="w-11 h-6 bg-[#2E7D32] rounded-full justify-center items-end px-0.5">
                  <View className="w-5 h-5 bg-white rounded-full" />
                </View>
              ) : (
                <View className="flex-row items-center gap-2">
                  {item.value && <Text className="text-sm text-[#757575]">{item.value}</Text>}
